...
def compute_nas_score(AnyPlainNet, random_structure_str, gpu, args):
    ...
    the_model = the_model.cuda(gpu)
    if len(the_model.block_list) != 5:
        return None

    try:
        if args.zero_shot_score == 'DDSAR':
...
def main(args, argv):
    ...
    for loop_count in range(args.evolution_max_iter):
        ...
        if loop_count >= 1 and loop_count % 500 == 0:
            ...
            logging.info('currrent best arch: {}'.format(popu_structure_list[np.argmax(popu_zero_shot_score_list)]))
