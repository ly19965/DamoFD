...
def compute_nas_score(gpu, model, mixup_gamma, resolution, batch_size, repeat, fp16=False):
    ...
    ddsar_score = model.forward_ddsar_score(input_size=[resolution, resolution])

    info['avg_nas_score'] = float(ddsar_score)
    info['std_nas_score'] = float(ddsar_score)
    info['avg_precision'] = float(ddsar_score)
