...
def get_select_student_channels_list(out_channels):
    #the_list = [out_channels * 2.5, out_channels * 2, out_channels * 1.5, out_channels * 1.25, out_channels]
    the_list = [ out_channels * 1.75, out_channels * 2, out_channels * 1.5, out_channels * 1.25,
                out_channels * 1,
                out_channels / 1.25, out_channels / 1.5, out_channels / 2, out_channels / 2.5]
    #the_list = [ out_channels * 1.5, out_channels * 1.25, out_channels * 1]
    #the_list = [min(128, max(8, x)) for x in the_list]
    #the_list = [min(256, max(8, x)) for x in the_list]
    the_list = [min(1024, max(8, x)) for x in the_list]
    the_list = [global_utils.smart_round(x, base=8) for x in the_list]
    the_list = list(set(the_list))
    the_list.sort(reverse=True)
    return the_list

def get_select_student_bottleneck_channels_list(out_channels):
...
def gen_search_space(block_list, block_id):
    ...
    if isinstance(the_block, super_blocks.SuperConvKXBNRELU):
        ...
        for student_block_type_list in seach_space_block_type_list_list:
            ...
            student_bottleneck_channels_list = get_select_student_bottleneck_channels_list(the_block.bottleneck_channels)
