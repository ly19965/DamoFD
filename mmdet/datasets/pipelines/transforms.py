        ...
        while True:
            ...
            for i in range(250):
                ...
                for key in results.get('bbox_fields', []):
                    ...
                    b_w_t = (boxes[:, 2] - boxes[:, 0] + 1) / cw * 640
                    b_h_t = (boxes[:, 3] - boxes[:, 1] + 1) / ch * 640
...
class RandomErasing(object):
    ...
    def __call__(self, results):
        ...
        orgH, orgW, orgC = results['img'].shape
        for attempt in range(100):
            ...
            if w < orgW and h < orgH:
                x1 = random.randint(0, orgH - h)
                y1 = random.randint(0, orgW - w)
                if orgC == 3:
                    results['img'][0, x1:x1 + h, y1:y1 + w] = self.mean[0]
                    results['img'][1, x1:x1 + h, y1:y1 + w] = self.mean[1]
                    results['img'][2, x1:x1 + h, y1:y1 + w] = self.mean[2]
                else:
                    results['img'][0, x1:x1 + h, y1:y1 + w] = self.mean[0]
                return results
